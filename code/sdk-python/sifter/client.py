...
class SiftHandle:
    ...
    def upload(
        ...
        """Upload documents. Pass a file/directory path, or raw bytes with a filename.

        on_conflict="replace" (default): re-upload replaces the existing document and
            re-triggers extraction.
        on_conflict="fail": raises an error if a document with the same name already exists.
        """
        import httpx
    ...
    def records(self, model: Optional[Type[M]] = None) -> list:
        """Return all extracted records, auto-paginating across pages.
...
class Sifter:
    ...
    def sift(self, path: str | Path, instructions: str) -> list[dict[str, Any]]:
        ...
        """
        s = self.create_sift(f"sift-temp-{int(time.time() * 1000)}", instructions)
        try:
            s.upload(path, on_conflict="replace")
            s.wait()
            return s.records()
        finally:
            s.delete()
